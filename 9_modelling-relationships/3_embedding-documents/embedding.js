...
const Course = mongoose.model('Course', new mongoose.Schema({
    ...
    author: {
        type: authorSchema,
        required: true // validation added here
    }
}));
...
async function deleteAuthorObjectFromCourse(courseId){
    // just need to use $unset as opposed to $set
    const course = await Course.update({_id: courseId}, {
        $unset: {
            'author': ''
        }
...
// createCourse('Node Course', new Author({name: 'Rupesh'}));
// updateAuthor('608a92c3929bcb41c0741bf0');
// deleteAuthorObjectFromCourse('608a92c3929bcb41c0741bf0');
listCourses();