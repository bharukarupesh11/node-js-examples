...
router.post('/', async (req, res) => {
    ...
    // Add the customer: Considering phone number unique. Here, while creating new Customer object mongoose talks to mongodb driver and then the driver itself generates the id i.e. '_id' for this movie document
    ...
    // customer = await customer.save(); // No need to reset customer here. Read the details in movies post router 
    await customer.save(); // This itself will work
...
router.put('/:id', async (req, res) => {
    ...
    if(!customer) return res.status(404).send(`The customer with "${req.params.id}" ID was not found!`);
